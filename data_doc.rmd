...
mult_reg <- mult_reg %>% drop_na(total_session_minutes) %>% dplyr::select(-c(x15_what_best_describes_your_current_childcare_arrangements,
...
model <- glm(good_standing ~ ., family=binomial(link='logit'),data=mult_reg,  maxit = 100) %>%  stepAIC(trace = FALSE) #added backward stepwise selection
...
```{r full_model_hispanic_only}

hispanics <- mult_reg %>%  dplyr::filter(are_you_of_hispanic_or_latino_origin == "Yes") %>% dplyr::select(-c(are_you_of_hispanic_or_latino_origin))
