...
hispanics <- mult_reg %>%  dplyr::filter(are_you_of_hispanic_or_latino_origin == "Yes") %>% select(-c(are_you_of_hispanic_or_latino_origin))

model_hispanic <- glm(good_standing ~ ., family=binomial(link='logit'),data=hispanics)
