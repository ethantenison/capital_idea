...
```{r full_model. Warning = FALSE, message= FALSE}
...
mult_reg <- mult_reg %>% drop_na(total_session_minutes) %>% dplyr::select(-c(are_you_of_hispanic_or_latino_origin,
...
```{r high_risk_model, Warning = FALSE, message= FALSE}

#Here I'm only looking at students who are are medium and high priority level 
mult_reg = surveys_casenotes %>%
          ungroup() %>% dplyr::filter(`Academic Priority Level` == "High Priority" | `Academic Priority Level` == "Medium Priority") %>% 
          dplyr::select(-c(`6a Are you enrolled in College Prep?`,
                           `6b Are you awaiting acceptance into a`,
                           `11d Please specify your OTHER financial`,`23a If yes, please explain`,
                           `Is this an Intake Applicant or a current`, `Survey Date`, `First`,
...
## The conversion
mult_reg[sapply(mult_reg, is.character)] <- lapply(mult_reg[sapply(mult_reg, is.character)], 
                                       as.factor)
...
# summary(model) the tidy model works better 
df_highrisk <- as.data.frame(tidy(model))




#This doesn't work because the data was taken from the academic score
...
split = sample.split(dfknn$good_standing, SplitRatio = 0.6)
...
tree$good_standing.Pred.Class <- ifelse(tree$good_standing.Pred > 0.9, "1", "0")
